...
# ╔═╡ 430a56c9-93ad-4513-b11c-2ac212109b12
md""" 
##### Solution

See [later in this lecture](#Challenge-Revisited:-Modeling-of-the-Old-Faithful-Data-Set).
"""
...
\begin{align}
   ...
   F[q^*] &= -\log p(x) \tag{= -log evidence}
\end{align}
```
"""

# ╔═╡ 670f83cb-66d6-40ab-9c42-50e022a9bd4c
md"""
In other words, global minimization of ``F[q]`` recovers Bayes rule _exactly_, namely, both the posterior and evidence are revealed.
"""
...
##### 3. Other constraints, e.g., the Expectation-Maximization (EM) algorithm

Beyond form and factorization constraints on ``q(z)``, several ad hoc algorithms have been developed to simplify VFE minimization for particular models. 
...
md"""
Figure caption: _The Bayesian posterior_ ``p(z|x)`` _(upper-right) is the posterior that would be obtained through executing Bayes rule, but unfortunately, Bayes rule is not tractable here. Instead, we propose a variational posterior_ ``q(z;\nu)``, _parameterized by_ ``\nu``. _The area inside the ellipsis represents the region reachable by choosing values for the parameter_ ``\nu``. _Note that_ ``p(z|x)`` _is not reachable. We start the FE minimization process by choosing an initial value_ ``\nu^{\text{init}}``, _which corresponds to the posterior_ ``q(z;\nu^{\text{init}})``, _as indicated in the figure. VFE minimization yields a final value_ ``\nu^{*}`` _that minimizes the KL divergence between_ ``q(z;\nu)`` _and_ ``p(z|x)``. 
...
function plotGMM(X::Matrix, clusters::Vector, γ::Matrix; kwargs...)
...
vfem_result = let
	...
	ν = fill(3.0, K, max_iterations)
	β = fill(1.0, K, max_iterations)
	α = fill(0.01, K, max_iterations)
	R = Array{Float64}(undef,K,N,max_iterations)
	M = Array{Float64}(undef,2,K,max_iterations)
	Λ = Array{Float64}(undef,2,2,K,max_iterations)
	clusters_vb = Array{Distribution}(undef,K,max_iterations) #clusters to be plotted
	#initialize prior distribution parameters
	M[:,:,1] = rand(MersenneTwister(43), 2, K) .* [4, 50] .+ [1, 50]
	for k in 1:K
	    Λ[:,:,k,1] = [1.0 0;0 0.01]
	    R[k,:,1] = 1/(K)*ones(N)
...
let
	i = iteration_vfem + 1
	plotGMM(
		X, vfem_result.clusters_vb[:,i], vfem_result.R[:,:,i];
		title=i == 1 ? "Initial situation" : "After $(iteration_vfem) iteration$(iteration_vfem > 1 ? "s" : "")",
		legend=nothing,
		size=(600,300),
	)
end
...
let
    ...
    current_clusters = vfem_result.clusters_vb[:, i]
    current_responsibilities = vfem_result.R[:, :, i]
