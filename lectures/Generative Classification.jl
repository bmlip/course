...
\end{align*}
```		
Because the quadratic term ``x_\bullet^T \hat{\Sigma}_k^{-1} x_\bullet`` is now class-dependent, the decision boundaries are given by quadratic equations in ``x``. Hence, the decision boundaries are generally (hyper-)ellipsoid surfaces.
...
##### Solution
  ...
  - [Later in this lecture](#Challenge-Revisited:-Apple-or-Peach).
"""
