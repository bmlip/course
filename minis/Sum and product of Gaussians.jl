...
When two variables are both Gaussian distributed and independent, their **sum** is Gaussian distributed, and their **product is not**.

Here are 50k samples from two variables: ``\color{blue} A`` (blue) and ``\color{red} B`` (red). You can control the mean of ``\color{blue} A``.
"""

# ╔═╡ 7104aab1-cc95-4839-bbfd-ec651319df3f
b = rand(Normal(3.0, 2.0), 51000)

# ╔═╡ 7709e0f8-1dc1-4f54-83df-6bd72757493f
md"""
Let's plot these samples in a histogram:
"""
...
md"""
The **sum** ``{\color{blue} A} + {\color{red} B}`` of two independent Gaussian distributed variables is Gaussian distributed.
"""

# ╔═╡ d91940c3-ebc2-487e-8e35-10bc31b08feb
md"""
The **product** ``{\color{blue} A} {\color{red} B}`` of two independent Guassian distributed variables is **not** Gaussian distributed!
"""

# ╔═╡ 20c5be37-6743-4a63-8197-7a3f27e61579


# ╔═╡ 5b409706-ac33-414d-a9b7-70a7329b5e66


# ╔═╡ 042e9625-cf96-47a5-ade1-8ef5efd4022d
...
hist(a; color="blue", label="A", legend=true, ylim=(0,10_000))

# ╔═╡ 1801ae3e-3d07-4c84-a685-497b468f7b16
hist(b; color="red", label="B", legend=true)

# ╔═╡ 44703df0-6c66-4090-87f6-77e9fcbde4bb
let
	sum = a .+ b

	hist(sum; color="purple", normalize=:pdf, label="A + B", legend=true)
	plot!(x -> pdf(Normal(mean(sum),std(sum)),x), label=nothing)
end
...
let
	...
	hist(prod; color="purple", normalize=:pdf, label="AB", legend=true)
	plot!(x -> pdf(Normal(mean(prod),std(prod)),x), label=nothing)
end
...
manifest_format = "2.0"
project_hash = "d28b8393375ad0152060fc40ac4f0a822d4584e8"
