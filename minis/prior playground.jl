...
Each ``x_i`` is independent, and takes values in ``\mathbb{R}``. Each ``x_i`` is iid:

```math
x_i \sim \text{Normal}(\mu, 0.5)
```

We want to **infer** the parameter ``\mu`` from our data!
...
# ╔═╡ f95bd162-a445-415f-adb8-d9d90040fd23
md"""Truncate distribution on interval ``[0,1]``? $(@bind truncate_0_1 CheckBox(; default=true))"""
...
cheader(s) = @htl """<span style="font-family: system-ui; font-size: 2rem; font-weight: bold;">$(s)</span>"""

# ╔═╡ efd519b1-c053-43bd-b279-3da80c6a082c
model(μ) = Normal(μ, 0.5)

# ╔═╡ 7ad2499e-65d3-4fb1-910d-60d3602add25
# prod_accurate(xs) = exp(sum(Iterators.map(log, xs); init=0.0))
...
# ╔═╡ 5775fec6-8f63-4b0b-969a-6fe41deaa8a6


# ╔═╡ 8d43c1cf-98af-4876-9768-e0f5f93cda20
function ∫(f, S)
	prob = IntegralProblem(
		(x, p) -> f(x), 
		S
	)
	solve(prob, HCubatureJL(initdiv=100); abstol=1e-2).u
end

# ╔═╡ 4356d379-55fc-4ae8-b5bc-0418e67b4462
...
"""
	...
	PlutoUI.Experimental.transformed_value(identity, h; get_initial_value=() -> initial_data)
end

# ╔═╡ 7362663c-a231-4e78-8e5b-0987cc2ad878
@bind data data_picker_1D(; xlim=(-.4,1.4))

# ╔═╡ 73feb66c-fb81-4e2e-a850-dc7ab213c650
const Layout = PlutoUI.ExperimentalLayout

# ╔═╡ ad18d493-04f1-4b54-8414-efdc41dea546
card(x) = Layout.Div([x]; style=Dict("background-color" => "#45914e42","border-radius" => "1rem", "min-height" => "6rem", "margin" => "0rem 1rem", "padding" => ".5rem"))
...
@bind carousel_index Carousel2([
	cheader("1. Uniform distribution")
	
	Layout.Div([
		cheader("2. Normal distribution")
		@bindname normal_mean Slider(-0.2:0.01:1.2; default=.5, show_value=true)
		@bindname normal_variance Slider(0.0001:0.001:1.2; default=.05, show_value=true)
	])

	
	Layout.Div([
		cheader("3. Mixed Normal distributions")
		@bindname mixing_ratio Slider(0.0:0.01:1.0; default=.2)
		md"---"
		@bindname normal_mean_a Slider(-0.2:0.01:1.2; default=.2, show_value=true)
		@bindname normal_variance_a Slider(0.0001:0.001:1.2; default=.03, show_value=true)
		md"---"
		@bindname normal_mean_b Slider(-0.2:0.01:1.2; default=.8, show_value=true)
		@bindname normal_variance_b Slider(0.0001:0.001:1.2; default=.02, show_value=true)
	])
	# md"# Two normals"
] .|> card)

# ╔═╡ f51a9c99-319d-42f1-bfa3-dd167903f4e2
prior_distribution = let
	dist = if ismissing(carousel_index) || carousel_index == 1
		Uniform(-10,10)
	elseif carousel_index == 2
		Normal(normal_mean, sqrt(normal_variance))
	elseif carousel_index == 3
		MixtureModel([
			Normal(normal_mean_a, sqrt(normal_variance_a))
			Normal(normal_mean_b, sqrt(normal_variance_b))
		], [mixing_ratio, 1.0 - mixing_ratio])
	end

	if truncate_0_1
		Distributions.truncated(dist; lower=0.0, upper=1.0)
	else
		dist
	end
end
...
manifest_format = "2.0"
project_hash = "38294edc630601a769bd4ab066bdeb497291e702"
...
[[deps.CommonSolve]]
git-tree-sha1 = "1bfa3e601ae1a441408f13ceec9a66cad88be4c4"
...
git-tree-sha1 = "c4be03bd0ecfa6cd192e2fedd243d472ae8c5ddc"
...
git-tree-sha1 = "4bd93ae7218466c38a2f2ec794b5201917cd3b43"
...
git-tree-sha1 = "6db54dd9d11f4aa27555e250d6b454bdde6f5dd1"
...
git-tree-sha1 = "758671acd7d22075f007cb450ece97ac9b56f14c"
...
git-tree-sha1 = "58cc83369cf114cc4c24f005346e0e7d1bf4bb64"
