...
```
is a so-called **equality** (or branching) node. The constraint ``f_=(x,x^\prime,x^{\prime\prime})`` enforces that ``x=x^\prime=x^{\prime\prime}`` **for every valid configuration**.
""", 
md"""
...
# ╔═╡ 679d05b5-9fee-46e7-9627-c886c34a612d
keyconcept("",
md"""
...
The Closing-the-box operation can alternatively be interpreted as **passing a message** from the newly created composite node to the rest of the graph.Closing the red box around ``f_a``, ``f_b`` and ``f_c`` leads to an outgoing message ``\overrightarrow{\mu}_{X_3}(x_3)`` for node ``f_c``, given by
...
In our research lab [BIASlab](http://biaslab.org) at TU Eindhoven, we are developing [RxInfer](http://rxinfer.com), which is a (Julia) toolbox for automating Bayesian inference by message passing in a factor graph.
...
# ╔═╡ 1f16c8db-f2da-4a72-8a69-743ec857ac3b
md"""
...
manifest_format = "2.0"
project_hash = "1d1644a5dd8f949f47ef0cb736ed9c8e8eeaf6cc"
